...
pub fn DirectedGraph(N: type, V: type, W: type, HashContext: type, comptime store_hash: bool) type {
    return struct {
        ...
        /// Depth limited (depth first) A^* iterator.
        /// Note: Experimental feature and is subject to change or removal.
        pub fn DLAStarIterator(Distance: type, HeuristicContext: type) type {
            return struct {
                const Entry = struct {
                    node: N,
                    tentative: Distance,
                ...
                const ResetOptions = struct {
                    direction: ?TraversalDirection = null,
                    start: N,
                    threshold: ?Distance = null,
                };
                
                const ThisIterator = @This();
                const Stack = ArrayHashMapUnmanaged(N, Distance, HashContext, store_hash);
                ...
                pub fn init(graph: *Self, options: InitOptions) Allocator.Error!ThisIterator {
                    var iter = ThisIterator{
                        ...
                        .context = options.context,
                        .direction = options.direction,
                        .heuristic = options.direction,
                        .graph = graph,
                        .stack = .{},
                        .threshold = options.threshold,
                    };
                    try iter.stack.append(iter.allocator, .{.node = options.start, .tentative = 0});
                    return iter;
                }
                
                pub fn next(iter: *ThisIterator) Allocator.Error!?Entry {
                    return while (iter.stack.popOrNull()) |entry| {
                        const estimate = entry.value + iter.heuristic(iter.context, entry.key);
                        if (estimate > iter.threshold) break .{.node = entry.key, .tentative = estimate};
	                    const neighbor_map = switch (iter.direction) {
	                        .sourcewards => iter.graph.node_map.get(entry.key).?.source_map,
	                        .targetwards => iter.graph.node_map.get(entry.key).?.target_map,
	                    };
	                    for (neighbor_map.keys(), neighbor_map.values()) |neighbor, weight| {
	                        const result = try iter.stack.getOrPut(iter.allocator, neighbor);
	                        if (result.found_existing) continue;
	                        const arithmetic_weight = if (W == ArithmeticWeight) weight else 1;
	                        const tentative = entry.value + arithmetic_weight;
	                        result.value_ptr.* = tentative;
	                    }
	                    break .{.node = entry.node, .estimate = estimate};
                    } else null;
                }
                
                pub fn peek(iter: *ThisIterator) ?Entry {
                    const count = iter.stack.count();
                    if (count == 0) return null;
                    const index = count - 1;
                    return .{.node = iter.stack.keys()[index], .tentative = iter.stack.values()[index]};
                }
   
                pub fn reset(iter: *ThisIterator, options: ResetOptions) void {
                    if (options.direction) |direction| iter.direction = direction;
                    if (options.threshold) |threshold| iter.threshold = threshold;
                    iter.start = options.start;
                    iter.stack.clearRetainingCapacity();
                    iter.stack.putAssumeCapacity(iter.start, 0);
                }
            };
        }
        /// Iterative deepening (depth first) A^* iterator.
        /// NOTE: Experimental and is subject to change or removal.
        pub fn IDAStarIterator(Distance: type, HeuristicContext: type) type {
            return struct {
                const Entry = DLAStarIterator(Distance, HeuristicContext).Entry;
                const HeuristicFn = *const fn (HeuristicContext, N) Distance;
	            const InitOptions = struct {
                    allocator: ?Allocator = null,
                    context: HeuristicContext,
	            ...
	            const ThisIterator = @This();
	            const Stack = ArrayList(Entry);
	            
	            dla: DLAStarIterator(Distance, HeuristicContext),
	            exceeded: bool,
	            start: N,
	            threshold: Distance,
	            ...
	            pub fn init(graph: *Self, options: InitOptions) Allocator.Error!ThisIterator {
	                return ThisIterator{
	                    .dla = try graph.dlaIterator(Distance, HeuristicContext, .{
	                        .allocator = options.allocator,
	                        .context   = options.context,
	                        .direction = options.direction,
	                        .heuristic = options.heuristic,
	            ...
	            pub fn next(iter: *ThisIterator) !?Entry {
	                if (try iter.dla.next()) |entry| {
	                    if (iter.dla.threshold < entry.tentative) {
	                        iter.threshold = if (!iter.exceeded)
	                            entry.tentative else
	                            @min(iter.threshold, entry.tentative);
	                        iter.exceeded = true;
	                    }
	                	return .{.node = entry.node, .tentative = entry.tentative};
	                } else {
	                   if (!iter.exceeded) return null;
	                   iter.exceeded = false;
	                   iter.dla.reset(.{.threshold = iter.threshold, .start = iter.start});
	                   return iter.next();
	                }
	            }
	            
	            pub fn peek(iter: *ThisIterator) ?Entry {
	                if (iter.dla.peek()) |entry| {
	                    return entry;
	                } else {
	                   if (!iter.exceeded) return null;
	                   iter.dla.reset(.{.threshold = iter.threshold, .start = iter.start});
        ...
        pub fn idaStarIterator(
            self: *Self,
            Distance: type,
            HeuristicContext: type,
            options: IDAStarIterator(Distance, HeuristicContext).InitOptions,
        ) Allocator.Error!IDAStarIterator(Distance, HeuristicContext) {
               return IDAStarIterator(Distance, HeuristicContext).init(self, options);
       }
    };
}
